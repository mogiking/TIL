...
# ## 커널 PCA 
#
# 비선형 문제를 풀기위한 PCA. 커널 SVM의 개념과 같이 고차원 공간에 $\mathbb{R}$을 투영하여 선형 문제로 변환한다. 이 과정에서 고차원 공간에서 연산이 늘어난다. 연산량을 줄이기 위해 **커널트릭**을 사용한다. 커널트릭을 사용하면 원본 특성 공간에서 두 고차원 특성 벡터의 유사도를 계산할 수 있다.
#
# $$k(x^{(i)},x^{(j)}) = \phi (x^{(i)})^T \phi (x^{(j)})$$
#
# $k$는 커널함수로 두 벡터사이의 점곱을 계산한다.
#
# 일반적인 계산은 다음과 같다.
#
# 1. 커널 행렬 $K$를 다음 식으로 계산한다.  $$k(x^{(i)},x^{(j)}) = \exp ( - \gamma ||x^{(i)} - x^{(j)}||)^2$$
#    샘플의 모든 쌍에 대해 구합니다.
# 1. 다음 식을 사용하여 커널 행렬 $K$를 중앙에 맞춥니다. $$K' = K -1_nK-K1_n+1_nK1_n$$
# 1. 고윳값 크기대로 내림차순으로 정렬하여 중앙에 맞춘 커널 행렬에서 최상위 k개의 고유벡터를 고릅니다. 표준 PCA와 다르게 고유 벡터는 주성분축이 아니며, 이미 이 축에 투영된 샘플입니다.
